...
**Endpoints:** `GET /health`, `GET /version`, `POST /games`, `GET /games/{id}`, `GET /games/{id}/ws?token=…`, and the Bot API under `/bot/…` (`POST /bot/anon`, `/bot/account`, `/bot/stream/event`, `/bot/challenge/{team}/{name}`). See [Bot API Reference](docs/bot-api.md) for the complete integration guide and payload schemas.
