...
## Webhook Push (Serverless Bots)

The push alternative to streams and polling (**registered bots only**): register an HTTPS callback once, and the
server POSTs to it whenever it is your turn — **the HTTP response body is your move**. A bot becomes a single
stateless HTTPS handler (a cloud function): no stream to hold, no timer to schedule, woken only when there is a
decision to make. Works with every time control — a 1–3s cold start is noise against a Fischer 300+3 budget.

Webhooks are enabled per server by the `WEBHOOK_TIMEOUT_SECONDS` env var (operators: `15` is a sensible cap; the
...
The URL must be **HTTPS** and resolve to a **public** address (loopback, RFC1918, link-local, CGNAT and IPv6-ULA
targets are rejected — the server refuses to be pointed at anyone's internal network). Before anything is stored,
the server performs an **ownership handshake**: it POSTs `{"type":"verification","nonce":"<random>"}` to the URL,
and the endpoint must answer `200` with `{"nonce":"<the same value>"}`. Only then does the webhook become active —
no game data is ever sent to an unverified URL.

- **Response:** `201 Created`
  ```json
  {"url": "https://my-function.example.com/dicechess", "secret": "3f9a…64 hex chars…c2"}
  ```
  `secret` is the per-bot HMAC key the server signs every delivery with — **shown exactly once**, keep it in your
  function's secret storage. Re-registering replaces both the URL and the secret.
- **Errors:**
  - `403 Forbidden` — anonymous or static caller (a registration perk, like token rotation).
  - `422 Unprocessable Entity` — URL policy violation or failed handshake (the body says which).
...
`state` is exactly the [`Snapshot.state`](#game-event-stream) object: `dfen` (the rolled dice ride in its 7th
field), `activeSeat`, `dicePending`, `clocks`, `commit`, and the inline [`legalMoves`](#legal-moves) tree under the
usual cap — when `legalMoves` is `null`, fetch [`GET /games/{id}/moves`](#get-legal-moves). The envelope is
self-sufficient: a pure function can pick its move from `legalMoves` alone, with no rules engine and no extra
round trip.
...
**Verify before trusting**: recompute the HMAC over `timestamp + "." + raw body` with your stored secret, compare
with the signature header, and reject timestamps outside a ±5-minute window (replay guard). The registration-time
verification request is the one delivery you cannot check — the secret is disclosed to you only after that
handshake succeeds.

**Respond within the timeout** with the same shape [`POST /bot/game/{id}/move`](#submit-turn-moves) accepts:
...
- Anything else — a timeout, a non-200, a malformed body, an illegal turn, or `{"moves": []}` — plays nothing:
  **your clock keeps running**, and the game forfeits on time exactly as if a polling bot had stopped polling.
  Delivery is **single-attempt** by design (no retries, no redelivery): the recovery budget for a transient glitch
  is your remaining clock, not a redelivery queue.

The wait for your answer is `min(server cap, your remaining clock)`. A webhook bot on the [rating
ladder](#join--leave-the-rating-ladder) is fully passive: the scheduler starts the games and the webhook delivers
the turns — the function needs no other integration. (Webhook bots do not contribute a client dice seed today; the
[provably-fair scheme](#provably-fair-dice) covers them with the documented participant-bound fallback, and the
seed endpoint stays available to hybrid bots that also hold streams.)
