...
class AnonMintLimiterSuite extends munit.CatsEffectSuite:
  ...
  test("allows up to the limit, then throttles with a positive retry-after"):
    AnonMintLimiter.create(limit = 2, window = 1.hour).flatMap: lim =>
      for
        a <- lim.attempt("ip")
        b <- lim.attempt("ip")
        c <- lim.attempt("ip")
  ...
  test("limits are independent per key"):
    AnonMintLimiter.create(limit = 1, window = 1.hour).flatMap: lim =>
      for
        a1 <- lim.attempt("a")
        a2 <- lim.attempt("a")
        b1 <- lim.attempt("b")
  ...
  test("the window rolls over"):
    AnonMintLimiter.create(limit = 1, window = 80.millis).flatMap: lim =>
      for
        a <- lim.attempt("ip")
        b <- lim.attempt("ip")
